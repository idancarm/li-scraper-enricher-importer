...
from .config import DEFAULT_UNIPILE_DSN, SCRAPE_DELAY_MIN_MS, SCRAPE_DELAY_MAX_MS
...
def _fetch_page(actor: dict, cursor: str | None = None) -> dict:
...
def scrape_contacts(actor: dict, run_id: str, max_pages: int | None = None) -> dict:
    """Scrape LinkedIn connections and insert into Supabase.
    ...
    while True:
        ...
        print(f"    Got {len(items)} contacts ({len(batch)} new, {len(items) - len(batch)} existing)")
        ...
        time.sleep(random.uniform(SCRAPE_DELAY_MIN_MS, SCRAPE_DELAY_MAX_MS) / 1000)
    ...
    print(f"  Scrape done: {total_scraped} total, {new_contacts} new, {skipped} existing")
    return stats
